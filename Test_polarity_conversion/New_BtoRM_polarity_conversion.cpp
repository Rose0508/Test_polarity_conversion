...
void New_BtoRM_polarity_conversion(char p[],int input, struct node* head) {
	int c[100];
	memset(c, -1, sizeof(c));
	...
	for (int i = 1; i <= input; i++) {
		...
		if (i < input) {
			...
			if (c[i] == 2) {
				...
				//cout << "执行left前" << endl;
				//print_list(head);
				find_left_individual(i, front, tail, sum, head);
				//cout << "执行left后" << endl;
				//print_list(head);
			}
			else if (c[i] == 1) {
				//cout << "执行right前" << endl;
				//print_list(head);
				find_right_individual(i, front, tail, sum, head);
				//cout << "执行right后" << endl;
				//print_list(head);
				//cout << "执行change前" << endl;
				//print_list(head);
				find_change_individual(i, front, tail, sum, head);
				//cout << "执行change后" << endl;
				//print_list(head);
			}
		}
